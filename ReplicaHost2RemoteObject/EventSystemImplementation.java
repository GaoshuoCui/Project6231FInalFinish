 ...
 */
package ReplicaHost2RemoteObject;
...
public class EventSystemImplementation  {
	...
	private SimpleEntry<Boolean, String> bookeventresult;
	private HashMap<String, ArrayList<String>> bookingschedule;
	private SimpleEntry<Boolean, String> Dropeventresult;
	//In memory database
 ...
 */
	public EventSystemImplementation(String city){
	...
	public synchronized String addEvent(String managerId, String eventId, String eventtype, int capacity) {
		...
		String reply = "";
		if (status == true) {
			reply="true- Add Event Finish!"+msg;
		}else {reply="false- Add Event fail!"+msg;}
		//this.udpToFE(status, "addEvent");
		System.out.println("EventSystemImpl- "+reply);
		return reply;
	}
	...
	public String removeEvent(String managerId, String eventId, String eventtype){
		...
		//this.udpToFE(status, "removeEvent");
		String reply = "";
		if (status == true) {
			reply="true- Remove Event Finish!"+msg;
		}else {reply="false- Remove Event fail!"+msg;}
		//this.udpToFE(status, "addEvent");
		System.out.println("EventSystemImpl- "+reply);
		return reply;
	}
	 ...
	 * @return 
	 */
	public HashMap<String, Integer> listEventAvailabilityOnServer(String managerId, String eventtype){
	...
	public synchronized String bookevent(String customerId, String eventId, String eventtype)
		...
		HashMap<String, ArrayList<String>> bookingSchedule = bookingschedule;
		//System.out.println("BS sche: " +bookingSchedule.toString());
		...
		if (city == eventCity) {
			//System.out.println("enroll in this city only" + cityEvents);
		...
		msg = result.getValue();
		//this.udpToFE(result, "bookevent");
		String reply = "";
		if (status == true) {
			reply="true- Book Event Finish!"+msg;
		}else {reply="false- Book Event fail!"+msg;}
		//this.udpToFE(status, "addEvent");
		System.out.println("EventSystemImpl- "+reply);
		bookeventresult = result;
		return reply;
	}
	...
	public String[] getbookingSchedule(String customerId){
		...
		//this.udpToFE(schedule, "getbookingSchedule");
		String[] arrayResult = schedule.toString().split(",");
		System.out.println("Result is : "+ schedule.toString());
	...
	public String dropevent(String customerId, String eventId) {
		...
		if (status == true) {
			reply="true- Drop Event Finish!"+msg;
		}else {reply="false- Drop Event fail!"+msg;}
		//this.udpToFE(status, "addEvent");
		System.out.println("EventSystemImpl- "+reply);
	...
	public String swapEvent(String customerId,  String neweventId, String neweventtype, String oldeventId, String oldeventtyped) {
		...
		if (!status) {
			//this.udpToFE(new SimpleEntry<Boolean, String>(status, msg), "swapEvent");
			String reply = "";
			if (status == true) {
				reply="true- Swap Event Finish!"+msg;
		...
		//Step6 drop old event
				...
				result2 = Dropeventresult;
				
				if (result2.getKey()) {//Drop old event good.
					//Step7: enroll in new event
					bookevent(customerId, neweventId, neweventtype);
					SimpleEntry<Boolean, String> any2 = bookeventresult;
					result2 = (SimpleEntry<Boolean, String>) any2;
	...
	private SimpleEntry<Boolean, String> atomicSwapOnThisServer(String customerId, String neweventId,
		...
		try {
			...
			int f,max=0;
			List<String> maxCityInSameMonth = new ArrayList<>();
			City CustomerCi = City.valueOf(customerId.substring(0, 3).toUpperCase());//MTL
	...
	public String[] listEventAvailability(String managerId, String eventtype) {
		...
		LOGGER.info(String.format(Constants.LOG_MSG, Constants.OP_LIST_EVENT_AVAILABILITY,
				Arrays.asList(managerId, eventtype), result != null, result));
		//this.udpToFE(result, "listEventAvailability");
