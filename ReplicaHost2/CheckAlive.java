...
import PortInformation.AlivePort;
import PortInformation.Replica;
import PortInformation.Replication;
...
public class CheckAlive implements Runnable{
    ...
    private void checkEcho(String RMaddress, int replica, int alivePort){
        try {
            ...
            ReplicaHost1.TimerForSleep timer = new ReplicaHost1.TimerForSleep(socket);
...
//            e.printStackTrace();
    ...
    private void TellRM(String RMaddress , int port) {
        try {
            InetAddress address = InetAddress.getByName(RMaddress);
            DatagramSocket socket = new DatagramSocket();
            byte[] data = FailureEnum.ServerCrash.toString().getBytes();
