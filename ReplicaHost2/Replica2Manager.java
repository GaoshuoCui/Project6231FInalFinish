...
public class Replica2Manager {
    ...
    public Replica2Manager(Logger logger) throws IOException {
        ...
        this.logger = logger;ReplicaId = "2";DeliveryNum = 0;SeqNumber = 0;
    ...
    private void restartReplica() throws IOException {
        ...
        Thread thread = new Thread(replica2);
        thread.start();
    }
    ...
    public void startRMListener(int RMPort) throws Exception {
        ...
        byte[] data = null;
        logger.info("Replica2 Manager Start");
        while(true)
        {
            data = new byte[1024];
    ...
    public static void main(String[] args) throws Exception {
        Logger replicaLogger = Logger.getLogger("RM_2.log");
        replicaLogger.setLevel(Level.ALL);

        //Replica2
        FileHandler FileHandler = new FileHandler("RM_2" + ".log");
        FileHandler.setFormatter(new LoggerFormatter());
        replicaLogger.addHandler(FileHandler);

        Replica2Manager RM = new Replica2Manager(replicaLogger);
        
        //Run listener for Task,Crash and backup for MTL/TOR/OTW servers.
        Runnable RMListenerTask = () -> {
            try {
                RM.startRMListener(Replica.REPLICA.replica2);
            } catch (Exception e) {
                e.printStackTrace();
            }
        };
...
enum ReplicaPort {
    REPLICA_PORT;
    final int port = 2222;
    final int backUpPort = 8082;
    final int softwareFail = 8882;
}

enum RMPortInfo{
    RM_PORT_INFO;
    final int recvMsg = 6002;
    final int execMsgToRplc = 2002;
    final int backUp = 5002;
    final int recvCrash = 7002;
}